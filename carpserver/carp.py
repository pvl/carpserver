...
class TextEncoder(nn.Module):
    ...
    def tok(self, string_batch):
        return self.tokenizer(string_batch, return_tensors="pt", padding=True).to(self.device)
...
class ContrastiveModel(nn.Module):
    def __init__(self, encA, encB, device="cuda"):
        super().__init__()
...
class TextReviewer:
    def __init__(self, n_ctx=512, use_half=True, device="cuda"):
    ...
    def tok(self, string_batch):
        for i, _ in enumerate(string_batch):
            if len(string_batch[i]) > self.n_ctx:
                string_batch[i] = string_batch[i][-self.n_ctx :]
    ...
    def get_passrev_logits(self, passages, reviews):
        ...
        with torch.no_grad():
            logits = self.model.getLogits([pass_tokens, pass_masks], [rev_tokens, rev_masks]).type(
                dtype=torch.float32
            )
        return logits
    ...
    def compute_softened_logits(self, passages, reviews1, reviews2, pairs=True):
        ...
        logits1 = torch.sum(self.get_passrev_logits(passages, reviews1), dim=-1).unsqueeze(
            0
        ) / float(len(reviews1))
        if pairs:
            logits2 = torch.sum(self.get_passrev_logits(passages, reviews2), dim=-1).unsqueeze(
                0
            ) / float(len(reviews2))
    ...
    def compute_logit(self, passages, reviews, soften=True, top_k=False, k=3, pairs=True):
        # Softens the classifiers by using paraphrasing.
        if soften:
            if pairs:
                review1_paraphrases = list(
                    set(self.pm.get(reviews[0], num_return_sequences=3) + [reviews[0]])
                )
                review2_paraphrases = list(
                    set(self.pm.get(reviews[1], num_return_sequences=3) + [reviews[1]])
                )
                ...
                softened_logits = self.compute_softened_logits(
                    passages,
                    review1_contextual + review1_paraphrases,
                ...
                review_paraphrases = list(
                    set(self.pm.get(reviews, num_return_sequences=3) + [reviews])
                )
                # print(review_paraphrases)

                review_contextual = list(map(lambda x: "[quote] " + x, review_paraphrases))
                softened_logits = self.compute_softened_logits(
                    passages, review_contextual + review_paraphrases, None, pairs=False
                )
    ...
    def review(self, story, reviews, soften=True, top_k=False, k=3, ret=True, pairs=True):
        output = []
        for rev in reviews:
            logit = self.compute_logit(story, rev, soften=soften, top_k=top_k, k=k, pairs=pairs)
            output.append(logit.cpu().item())
        return {
            "reviews": reviews,
            "scores": output,
            "best": reviews[np.argmax(output)],
        }
